...
- `twig-grepper` — Maps `__TwigTemplate_*` class names from a PHP backtrace to the original Twig template files by searching the Drupal Twig cache (`sites/default/files/php/twig`). Accepts a file argument (`ddev twig-grepper backtrace.txt`) or piped input (`pbpaste | ddev twig-grepper`). Prints each resolved template path in green, or "not in cache" in red if the compiled class is not found. Useful for turning cryptic backtrace class identifiers into actionable template paths.
