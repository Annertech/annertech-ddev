...
- `twig-grepper` — Maps `__TwigTemplate_*` class names from a PHP backtrace to their original Twig template source paths. Accepts a file argument or reads from stdin (e.g. `pbpaste | ddev twig-grepper`, `wl-paste | ddev twig-grepper`). Extracts every `__TwigTemplate_<hash>` token from the input, searches the compiled Twig cache (`$DOCROOT/sites/default/files/php/twig`) for a matching file, and prints the source template path (relative to the docroot) in green, or "not in cache" in red, with the raw class hash shown in grey beneath each match. Useful when debugging Twig exceptions whose backtraces show compiled-class names rather than template file paths.
